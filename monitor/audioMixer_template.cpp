...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	if(!foundSlot) {
		printf("No Free Slot Found - AudioMixer_queueSound\n");
	}
...
static void fillPlaybackBuffer(short *playbackBuffer, int size)
{
	...
	// NOT GOING UP TO 120506, instead only going to 734
	// loop over soundBites
	for(int i = 0; i < MAX_SOUND_BITES; i++) {

		// if there is an audio clip to play
		// printf("i = %i \n", i);
		pthread_mutex_lock(&audioMutex);
		if(soundBites[i].pSound != NULL) {
			// printf("playbackBufferSize = %i \n", size);
			// how much of the audio clip is left to play
			int lenLeftToPlay = soundBites[i].pSound->numSamples - soundBites[i].location;

			// how large is the audio clip left-to-play relative to the size of the playback buffer
			int lenToCopy = size - lenLeftToPlay;
			if(lenToCopy <= 0) {
				lenToCopy = size;	
			} else {
				lenToCopy = lenLeftToPlay;
			}

			// loop over for each spot that we have to copy values for
			int k = soundBites[i].location;
			for(int j = 0; j < lenToCopy; j++) {
				// printf("soundBites[%i].pSound->pData[%i] = %d \n", i, j, soundBites[i].pSound->pData[j]);
				int index = j + k;
				short psoundVal = soundBites[i].pSound->pData[index];
				// clip the total val
				int totalVal = playbackBuffer[j] + psoundVal;
				if(totalVal > SHRT_MAX) {
					totalVal = SHRT_MAX;
				} else if(totalVal < SHRT_MIN) {
					totalVal = SHRT_MIN;
				}

				short val = (short)totalVal;
				playbackBuffer[j] = val;
				// printf("playbackBuffer[%i] = %d \n\n", j, playbackBuffer[j]);
			}

			// if we've played the entire soundbite, free up the slot
			soundBites[i].location += lenToCopy;
